...
function seed() {
  ...
  for (const v of venues) data.venues.push({ id: nid('venues'), status: 'active', ...v });
  ...
  function makeTeam(name, sport, i) {
    const team = {
      ...
      area: 'Sector ' + (i + 1),
      homeGround: sport.key === 'futsal' ? venues[i % 2].name : venues[2 + (i % 2)].name,
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const venuePool = i % 2 === 0 ? [venues[0].id === 1 ? data.venues[0] : data.venues[0]] : [data.venues[2]];
  ...
  console.log('Admin login -> email: admin@sportshub.com / password: Admin@123');
